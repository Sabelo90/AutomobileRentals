...
namespace AutomobileRentalsCore.Services
{
    ...
    {
        private readonly IBookingResource _resource;
        public BookingService(IBookingResource _resource)
        {
            _resource = _resource;
        }
        ...
        public async Task<BookingModel> DeleteAsync(BookingModel model)
        {
            try
            {
                //todo.. Aunthetication and Authorisation
                ...
                if (!canDelete)
                {
                    ...
                    throw new Exception($"Delete disallowed for person with id: {model.bok_id}");
                }
        ...
        public async Task<BookingModel> PostAync(BookingModel model)
        {
            try
            {
                //todo.. Aunthetication and Authorisation 
        ...
        public async Task<BookingModel> PutAsync(BookingModel model)
        {
            try
            {
                //todo.. Aunthetication and Authorisation 
                ...
                {
                    //todo.. find a better way to implement this
                    throw new Exception($"Update disallowed for model with id: {model.bok_id}");
                }
                ...
                if (!canSave)
                {
                    throw new Exception($"Update disallowed for person with id: {model.bok_id}");
                }
        ...
        public Task<bool> onDeleteListCompletedAsync(BookingModel[] models)
        {
